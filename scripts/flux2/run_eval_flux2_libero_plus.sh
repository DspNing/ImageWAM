...
FLUX2_VARIANT="${FLUX2_VARIANT:-9b}" # 4b | 9b
...
export CONDA_ENV_PYTHON="${CONDA_ENV_PYTHON:-/home/NingZijian/miniconda3/envs/imagewam/bin/python}"
...
COMMON+=(
  ...
  model.qwen_context_len="${QWEN_CONTEXT_LEN:-128}"
  data.train.qwen_text_cache_format=qwen3_flux2
  model.flux2_lora_config.enabled="${FLUX2_LORA_ENABLED:-false}"
  MULTIRUN.task_sample_ratio="${TASK_SAMPLE_RATIO:-0.2}"
